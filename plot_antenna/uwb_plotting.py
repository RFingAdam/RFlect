...
def plot_sff_vs_angle(angles, sff_values):
    ...
    bars = ax.bar([str(a) for a in angles], sff_values, color=colors, edgecolor='black', linewidth=0.5)
    ...
    ax.axhline(mean_sff, color='navy', linestyle='--', linewidth=1.5,
               label=f'Mean SFF = {mean_sff:.3f}')
...
def plot_impulse_response(time_s, h_t):
    ...
    if len(indices) >= 2:
        ...
        ax.annotate(f'Width = {width:.2f} ns',
...
def plot_input_vs_output_pulse(time_s, input_pulse, output_pulse, sff, delay_s):
    ...
    # Normalize
    inp = input_pulse / np.max(np.abs(input_pulse)) if np.max(np.abs(input_pulse)) > 0 else input_pulse
    out = output_pulse / np.max(np.abs(output_pulse)) if np.max(np.abs(output_pulse)) > 0 else output_pulse

    # Trim output to same length
    n = min(len(inp), len(out))

    ax.plot(t_ns[:n], inp[:n], 'b-', linewidth=1.0, label='Input Pulse')
    ax.plot(t_ns[:n] + delay_ns, out[:n], 'r--', linewidth=1.0,
    ...
    if len(above_thresh) > 0:
        ...
        right = min(n, above_thresh[-1] + margin + int(delay_ns / (t_ns[1] - t_ns[0]) if len(t_ns) > 1 else 0))
        right = min(right, n)
...
def plot_s11_vswr(freq_hz, s11_dB, vswr, bandwidth_info):
    ...
    if bandwidth_info['bandwidth_hz'] > 0:
        ...
        ax1.axvspan(f_start, f_stop, alpha=0.15, color='green',
...
def plot_group_delay_variation(freq_hz, angles_dict):
    ...
    ax.axhline(mean_var, color='red', linestyle='--',
               label=f'Mean variation = {mean_var:.1f} ps')
