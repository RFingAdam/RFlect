...
def read_farfield_file(path: str) -> tuple[float, pd.DataFrame]:
    ...
    df = pd.read_csv(StringIO("".join(data)), delim_whitespace=True, header=None, engine="python")
    if df.shape[1] != 8:
...
def plot_group_delay_vs_phi(phis, delays):
    ...
    plt.axhline(np.mean(delays), ls="--", label=f"Mean = {np.mean(delays):.2f} ns")
...
def plot_group_delay_vs_frequency_annotated(freqs, τ0):
    ...
    plt.hlines([τ_low, τ_hi], freqs[0] / 1e9, freqs[-1] / 1e9, colors="gray", linestyles="--")
    plt.annotate(
        "",
        xy=(freqs[-1] / 1e9, τ_hi),
    ...
    plt.text(freqs[-1] / 1e9 + 0.005, (τ_low + τ_hi) / 2, f"{Δτ_ps:.1f} ps", va="center")
...
def plot_envelope_vs_frequency(freqs, τ, plane_name):
    ...
    meanτ = τ.mean(axis=0)
    plt.figure(figsize=(8, 4))
    plt.fill_between(freqs / 1e9, minτ, maxτ, alpha=0.3, label=f"{plane_name} envelope")
...
def plot_cdf_vs_threshold_spread(angles_phi, angles_theta, τ_phi, τ_theta):
    ...
    plt.axvline(Δτ_phi0, color="C1", ls="--", label=f"φ‑plane Δτ = {Δτ_phi0:.1f} ps")
...
def compute_group_delay_map(files, dom_pol="Phi"):
    ...
    for k, (_, df) in enumerate(freq_df):
        ...
        P_deg[:, :, k] = grid.reindex(index=all_thetas, columns=all_phis).values
...
def plot_io_pulse(tt: np.ndarray, inp: np.ndarray, out_raw: np.ndarray, delay_ns: float) -> None:
    """
...
def plot_fidelity_vs_phi(phis: np.ndarray, fids: np.ndarray, band: tuple[float, float]) -> None:
...
def plot_fidelity_vs_theta(thetas: np.ndarray, fids: np.ndarray, band: tuple[float, float]) -> None:
    """Same as φ but vs θ."""
    fmin, fmax = band
    plt.figure(figsize=(6, 4))
    ...
    def proceed():
        ...
        if mode == "GroupDelay":
            ...
            files = filedialog.askopenfilenames(
                title="Select CST far-field .txt files", filetypes=[("Text files", "*.txt")]
            )
            if not files:
            ...
            phis, delays, all_delays = compute_group_delay(
                files, dominant_polarization=var_pol.get()
            )
            ...
            angles_theta, _, P_theta = compute_plane_data(
                files, f"Phase({var_pol.get()})[deg.]", "Phi [deg.]", 0.0, "Theta [deg.]"
            )
            ...
            F0, t_lin, pulse, R_lin, delay_s = compute_fidelity_at_boresight(
                list(files), dom_pol=var_pol.get(), theta0=90.0, phi0=0.0, band=None, Nfft=NFFT_FID
            )
            ...
            if not common_fs:
                messagebox.showerror(
                    "No match", "No overlapping frequencies between ANT1 and ANT2 files."
                )
                return

            # sample according to Low/Mid/High/Single
            if var_ecc_freq.get() == "Sample3Freq":
                freqs = [common_fs[0], common_fs[len(common_fs) // 2], common_fs[-1]]
            elif var_ecc_freq.get() == "SingleFreq":
                idx = simpledialog.askinteger(
                    "Pick frequency index", f"Select index 0–{len(common_fs)-1}:"
                )
