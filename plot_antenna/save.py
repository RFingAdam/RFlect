...
def detect_measurement_type(filename):
    ...
    if 'polar' in filename_lower or 'ar_' in filename_lower or 'tilt' in filename_lower or 'sense' in filename_lower or 'xpd' in filename_lower:
        return 'polarization'
...
class RFAnalyzer:
    ...
    def analyze_image(self, image_path, measurement_type=None, is_paired=False):
        ...
        """
        if self.use_ai:
            analysis = self.send_to_openai(image_path, self.project_context, measurement_type, is_paired)
    ...
    def analyze_image_batch(self, image_paths, frequency, batch_type, measurement_type=None):
        ...
        if not self.use_ai or not image_paths:
            return self._generate_batch_placeholder(image_paths, frequency, batch_type, measurement_type)

        # Limit to first 4 images for batch analysis (API limits on image count)
        images_to_analyze = image_paths[:4]

        # Build batch-specific prompt
        prompt = self._build_batch_prompt(frequency, batch_type, measurement_type, len(images_to_analyze))
...
3. **Performance Notes**: Notable observations"""
        ...
        response_style = config.AI_RESPONSE_STYLE if hasattr(config, 'AI_RESPONSE_STYLE') else 'concise'
...
• Coverage: [assessment]"""
    ...
    def _send_batch_to_responses_api(self, image_paths, ai_model, prompt):
        ...
        reasoning_effort = config.AI_REASONING_EFFORT if hasattr(config, 'AI_REASONING_EFFORT') else 'low'
        text_verbosity = config.AI_TEXT_VERBOSITY if hasattr(config, 'AI_TEXT_VERBOSITY') else 'auto'
...
• Pattern: Review for key characteristics"""
    ...
    def _extract_metrics(self, analysis_text, image_path):
        ...
        trp_matches = re.findall(r'TRP[:\s]+([+-]?\d+(?:\.\d+)?)\s*dBm', analysis_text, re.IGNORECASE)
        for trp in trp_matches:
            self.measurement_stats['trp_values'].append(float(trp))
        
        # Extract gain values (dBi/dBd)
        gain_matches = re.findall(r'(?:gain|peak)[:\s]+([+-]?\d+(?:\.\d+)?)\s*dBi?', analysis_text, re.IGNORECASE)
        for gain in gain_matches:
            gain_val = float(gain)
            if self.measurement_stats['max_gain'] is None or gain_val > self.measurement_stats['max_gain']:
                self.measurement_stats['max_gain'] = gain_val
            if self.measurement_stats['min_gain'] is None or gain_val < self.measurement_stats['min_gain']:
                self.measurement_stats['min_gain'] = gain_val
        
        # Track issues/characteristics mentioned
        keywords = ['asymmetric', 'null', 'ripple', 'high AR', 'poor XPD', 'low efficiency', 'variation']
    ...
    def generate_executive_summary(self):
        ...
        if 'active' in type_counts:
            type_desc.append(f"{type_counts['active']} active (TRP) measurement{'s' if type_counts['active'] > 1 else ''}")
        if 'passive' in type_counts:
            type_desc.append(f"{type_counts['passive']} passive gain pattern{'s' if type_counts['passive'] > 1 else ''}")
        if 'polarization' in type_counts:
            type_desc.append(f"{type_counts['polarization']} polarization analysis plot{'s' if type_counts['polarization'] > 1 else ''}")
        ...
        if self.measurement_stats['trp_values']:
            avg_trp = sum(self.measurement_stats['trp_values']) / len(self.measurement_stats['trp_values'])
            if len(self.measurement_stats['trp_values']) > 1:
                ...
                highlights.append(f"TRP ranging from {min_trp:.1f} to {max_trp:.1f} dBm (average: {avg_trp:.1f} dBm)")
            else:
        ...
        if self.measurement_stats['issues_found']:
            ...
            paragraphs.append(f"Analysis identified notable pattern characteristics including {issues_text}. Refer to individual measurement sections for detailed assessment.")

        # Closing
        paragraphs.append("Detailed findings and quantitative performance data are presented in the following sections.")
    ...
    def generate_conclusions(self):
        ...
        if self.measurement_stats['frequencies']:
            ...
            if num_freqs > 1:
                conclusions.append(f"Multi-band characterization completed across {num_freqs} frequency points. Individual band performance should be validated against band-specific requirements.")

        # TRP/Power analysis
        if self.measurement_stats['trp_values']:
            if len(self.measurement_stats['trp_values']) > 1:
                trp_range = max(self.measurement_stats['trp_values']) - min(self.measurement_stats['trp_values'])
                if trp_range > 6:
                    conclusions.append(f"TRP variation of {trp_range:.1f} dB observed across frequency bands. Verify compliance with regulatory power limits and system link budget margins.")

        # Gain performance
        if self.measurement_stats['max_gain'] is not None:
            conclusions.append(f"Peak gain of {self.measurement_stats['max_gain']:.1f} dBi documented. Confirm alignment with product datasheet specifications and application requirements.")

        # Pattern quality observations
        asymmetric_count = sum(1 for result in self.analysis_results if 'asymmetric' in result['analysis'].lower())
        if asymmetric_count > 0:
            conclusions.append(f"Pattern asymmetry noted in {asymmetric_count} measurement(s). Evaluate impact on intended coverage requirements and mounting orientation.")

        # Null analysis
        null_keywords = ['deep null', 'pronounced null', 'significant null', 'null depth']
        null_count = sum(1 for result in self.analysis_results if any(word in result['analysis'].lower() for word in null_keywords))
        if null_count > 0:
            conclusions.append(f"Notable pattern nulls identified in {null_count} measurement(s). Verify null positions are acceptable for the intended installation configuration.")
        ...
        conclusions.append("Cross-reference measurement results with electromagnetic simulation data to validate design correlation.")
        conclusions.append("Retain measurement data and chamber calibration records for product qualification documentation.")

        return conclusions
    
    def send_to_openai(self, image_path, project_context=None, measurement_type=None, is_paired=False):
        """
        ...
        # Use appropriate API
        if is_gpt5_model:
            return self._send_to_responses_api(base64_image, ai_model, project_context, measurement_type, is_paired)
        else:
            return self._send_to_chat_completions_api(base64_image, ai_model, project_context, measurement_type, is_paired)
...
"""
        ...
        include_recommendations = config.AI_INCLUDE_RECOMMENDATIONS if hasattr(config, 'AI_INCLUDE_RECOMMENDATIONS') else False
...
"""
    ...
    def _send_to_chat_completions_api(self, base64_image, ai_model, project_context, measurement_type, is_paired=False):
        """
        ...
        try:
            ...
            return "**AI Analysis Unavailable** - Network error. Report will use placeholder captions."
    ...
    def _send_to_responses_api(self, base64_image, ai_model, project_context, measurement_type, is_paired=False):
        """
        ...
        reasoning_effort = config.AI_REASONING_EFFORT if hasattr(config, 'AI_REASONING_EFFORT') else "low"
        text_verbosity = config.AI_TEXT_VERBOSITY if hasattr(config, 'AI_TEXT_VERBOSITY') else "auto"
        generate_summary = config.AI_GENERATE_REASONING_SUMMARY if hasattr(config, 'AI_GENERATE_REASONING_SUMMARY') else False
...
def generate_report(doc_title, images, save_path, analyzer, logo_path=None, 
                    metadata=None, include_toc=True, include_summary=True):
    """
    ...
    BRAND_PRIMARY = RGBColor(*config.BRAND_PRIMARY_COLOR) if config.BRAND_PRIMARY_COLOR else RGBColor(70, 130, 180)
    BRAND_DARK = RGBColor(*config.BRAND_DARK_COLOR) if config.BRAND_DARK_COLOR else RGBColor(50, 50, 50)
    BRAND_LIGHT = RGBColor(*config.BRAND_LIGHT_COLOR) if config.BRAND_LIGHT_COLOR else RGBColor(128, 128, 128)
    ...
    report_subtitle = config.REPORT_SUBTITLE if hasattr(config, 'REPORT_SUBTITLE') else "Antenna Measurement & Analysis Report"
    ...
    def add_branded_heading(doc, text, level=1):
        ...
        for run in heading.runs:
            if level == 1:
                ...
                run.font.size = Pt(config.HEADING1_FONT_SIZE if hasattr(config, 'HEADING1_FONT_SIZE') else 18)
            else:
                run.font.color.rgb = BRAND_DARK
                run.font.size = Pt(config.HEADING2_FONT_SIZE if hasattr(config, 'HEADING2_FONT_SIZE') else 14)
            run.font.bold = True
        return heading
    ...
    if not logo_path and hasattr(config, 'LOGO_FILENAME') and config.LOGO_FILENAME:
        ...
        possible_logo_paths = [
            os.path.join(os.path.dirname(os.path.dirname(__file__)), 'assets', config.LOGO_FILENAME),
    ...
    if report_subtitle:
        ...
        subtitle_run.font.size = Pt(config.SUBTITLE_FONT_SIZE if hasattr(config, 'SUBTITLE_FONT_SIZE') else 14)
    ...
    if include_toc:
        ...
        toc_para = doc.add_paragraph('< Table of Contents will be generated when document is opened in Word >')
    ...
    for group_name, group_images in image_groups.items():
        ...
        sorted_freqs = sorted(freq_grouped.keys(), key=lambda x: float(x.replace('MHz', '').replace('GHz', '000')) if x != 'unknown' else 0)
        ...
        for freq in sorted_freqs:
            ...
            if freq_data['3d_plots']:
                ...
                for img_tuple in three_d_images:
                    ...
                    display_name = filename.replace('_1of2', ' (View 1 of 2)').replace('_2of2', ' (View 1 of 2)') if is_paired else filename
                    ...
                    if is_paired and paired_img:
                        ...
                        display_name2 = paired_filename.replace('_2of2', ' (View 2 of 2)').replace('_1of2', ' (View 2 of 2)')
    ...
    if analyzer.use_ai:
        ...
        doc.add_paragraph("Based on the measurement results presented in this report:")
        doc.add_paragraph("• Review all performance metrics against specification requirements", style='List Bullet')
        doc.add_paragraph("• Verify antenna performance meets application needs across the operational bandwidth", style='List Bullet')
        doc.add_paragraph("• Consider additional measurements or design iterations if performance gaps are identified", style='List Bullet')
...
def save_to_results_folder(selected_frequency, freq_list, scan_type,
                           hpol_path, vpol_path, active_path,
                           cable_loss, datasheet_plots,
                           axis_mode, zmin, zmax,
                           word=False, logo_path=None):    # Initialize the GUI
    root = Tk()
