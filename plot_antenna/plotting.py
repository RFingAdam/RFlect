...
def plot_additional_active_polar_plots(phi_angles_rad, theta_angles_rad, total_power_dBm_2d, frequency, save_path=None):
    ...
    idx_phi_270 = np.argwhere(np.isclose(phi_angles_rad, 3 * np.pi / 2, atol=1e-6)).flatten()  # 270° = 3π/2
    idx_phi_90 = np.argwhere(np.isclose(phi_angles_rad, np.pi / 2, atol=1e-6)).flatten()       # 90° = π/2
...
def plot_active_3d_data(theta_angles_deg, phi_angles_deg, total_power_dBm_2d,
                        ...
                        frequency, power_type='total', interpolate=True,
                        axis_mode='auto', zmin: float = -15.0, zmax: float = 15.0, save_path=None):
    """
    ...
    surf = ax.plot_surface(
        ...
        linewidth=0.5, antialiased=True, shade=False, rstride=1, cstride=1, zorder=1
    )
    # Set the view angle
    ...
    ax.plot([0, axis_length], [0, 0], [0, 0], color='green', linewidth=2.5, linestyle='-', alpha=0.9)
    ax.quiver(axis_length - arrow_head, 0, 0, arrow_head, 0, 0, color='green',
              arrow_length_ratio=0.5, linewidth=2)
    ax.text(axis_length * 1.08, 0, 0, 'X', color='green', fontsize=14, fontweight='bold', ha='center')

    # Y-axis (red) - extends in positive Y direction
    ax.plot([0, 0], [0, axis_length], [0, 0], color='red', linewidth=2.5, linestyle='-', alpha=0.9)
    ax.quiver(0, axis_length - arrow_head, 0, 0, arrow_head, 0, color='red',
              arrow_length_ratio=0.5, linewidth=2)
    ...
    ax.quiver(0, 0, axis_length - arrow_head, 0, 0, arrow_head, color='blue',
              arrow_length_ratio=0.5, linewidth=2)
    ax.text(0, 0, axis_length * 1.08, 'Z', color='blue', fontsize=14, fontweight='bold', ha='center')
...
# Plot passive data
def plot_2d_passive_data(theta_angles_deg, phi_angles_deg, v_gain_dB, h_gain_dB,
                         Total_Gain_dB, freq_list, selected_frequency, datasheet_plots,
                         save_path=None):
    """
    ...
    unique_theta = np.unique(theta_angles_deg[:, 0]) if theta_angles_deg.ndim > 1 else np.unique(theta_angles_deg)
    unique_phi = np.unique(phi_angles_deg[:, 0]) if phi_angles_deg.ndim > 1 else np.unique(phi_angles_deg)
    ...
    if save_path:
        eff_db_path = os.path.join(
            save_path,
            f"efficiency_db_{min_freq}-{max_freq}MHz.png"
        )
    ...
    if save_path:
        eff_percent_path = os.path.join(
            save_path,
            f"efficiency_percent_{min_freq}-{max_freq}MHz.png"
        )
    ...
    fig.gca().grid(True, which='both', linestyle='--', linewidth=0.5)
    if save_path:
        total_gain_path = os.path.join(
            save_path,
            f"total_gain_{min_freq}-{max_freq}MHz.png"
        )
...
def plot_passive_3d_component(theta_angles_deg, phi_angles_deg, h_gain_dB, v_gain_dB, Total_Gain_dB,
                              freq_list, selected_frequency, gain_type, 
                              axis_mode='auto', zmin: float = -15.0, zmax: float = 15.0, save_path=None, shadowing_enabled=False, shadow_direction='-X'):
    """
...
def process_vswr_files(file_paths,
                       saved_limit1_freq1, saved_limit1_freq2, saved_limit1_start, saved_limit1_stop,
                       saved_limit2_freq1, saved_limit2_freq2, saved_limit2_start, saved_limit2_stop):
    import matplotlib.pyplot as plt
    import os
    import pandas as pd
    ...
    if '!' in first_line:
        ...
        unique_columns = sorted({c for d in all_data for c in d.columns if c != '! Stimulus(Hz)'},
        ...
        for ax, column in zip(axes, unique_columns):
            ...
            if saved_limit1_freq1 != 0 and saved_limit1_freq2 != 0:
                ax.plot([saved_limit1_freq1, saved_limit1_freq2],
                        [saved_limit1_start, saved_limit1_stop],
                        color='red', linestyle='--', linewidth=2, zorder=100, label='Limit 1')

            if saved_limit2_freq1 != 0 and saved_limit2_freq2 != 0:
                ax.plot([saved_limit2_freq1, saved_limit2_freq2],
                        [saved_limit2_start, saved_limit2_stop],
                        color='green', linestyle='--', linewidth=2, zorder=100, label='Limit 2')
...
def plot_polarization_2d(theta_deg, phi_deg, ar_db, tilt_deg, sense, xpd_db, frequency, save_path=None):
    """
    ...
    c3 = ax3.contourf(phi_mesh, theta_mesh, sense, levels=[-1.5, -0.5, 0.5, 1.5], 
                      colors=['blue', 'white', 'red'], alpha=0.7)
    ...
    if save_path:
        fig.savefig(os.path.join(save_path, f'polarization_2D_{frequency}MHz.png'), dpi=300, bbox_inches='tight')
        plt.close(fig)
    else:
        plt.show()
...
def plot_polarization_3d(theta_deg, phi_deg, ar_db, tilt_deg, sense, frequency, save_path=None):
    ...
    interp_ar = RegularGridInterpolator((theta_deg, phi_deg_wrapped), ar_db_wrapped, 
                                        method='linear', bounds_error=False, fill_value=np.nan)
    interp_sense = RegularGridInterpolator((theta_deg, phi_deg_wrapped), sense_wrapped, 
    ...
    surf1 = ax1.plot_surface(x, y, z, facecolors=cmap_viridis(ar_display/30), 
    ...
    surf2 = ax2.plot_surface(x, y, z, facecolors=cmap_rdbu(sense_colors), 
                             rstride=1, cstride=1, linewidth=0,
                             antialiased=True, shade=True, alpha=0.95)
    ax2.set_xlabel('X')
    ax2.set_ylabel('Y')
    ax2.set_zlabel('Z')
    ax2.set_title(f'Polarization Sense (3D) @ {frequency} MHz\nRed=LHCP, Blue=RHCP', 
    ...
    if save_path:
        fig.savefig(os.path.join(save_path, f'polarization_3D_{frequency}MHz.png'), dpi=300, bbox_inches='tight')
        plt.close(fig)
    else:
        plt.close('all')
        plt.show()