...
def apply_directional_human_shadow(gain_dB, theta_deg, phi_deg,
                                   frequency_mhz,
                                   target_axis='+X',
    ...
    delta = np.sqrt(2 / (omega * 4e-7 * pi * np.sqrt(1 + (sigma / (omega * epsilon)) ** 2)))  # skin depth
    ...
    for result in pol_results:
        ...
        if format == 'csv':
            ...
            with open(filepath, 'w') as f:
                ...
                f.write("="*80 + "\n\n")
                f.write(f"{'Theta':>8} {'Phi':>8} {'AR(dB)':>10} {'Tilt(°)':>10} "
                       f"{'Sense':>6} {'XPD(dB)':>10}\n")
                f.write("-"*80 + "\n")
                
                for i in range(n_points):
                    sense_str = "LHCP" if result['sense'][i] > 0 else "RHCP"
                    f.write(f"{result['theta'][i]:8.2f} {result['phi'][i]:8.2f} "
